...
if (Sys.info()['sysname'] == "Windows") {
  setwd("G:/Project/实证/关联框架/Python脚本/SPAADIA分析脚本")
} else {
...
# 读取数据
cat("读取数据...\n")
data <- read.csv("temp_data_for_r.csv", stringsAsFactors = FALSE, fileEncoding = "UTF-8")
...
if (!is.null(results_nested)) {
  ...
  # Windows兼容的文件写入
  con <- file("three_level_icc_results.json", "w", encoding = "UTF-8")
  writeLines(json_output, con)
  close(con)
