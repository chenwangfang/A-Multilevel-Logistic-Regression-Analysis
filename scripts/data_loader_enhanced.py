...
# 数据路径配置
BASE_DIR = Path(__file__).parent.parent.parent  # 从当前脚本位置推导基础路径
...
class SPAADIADataLoader:
    ...
    def _load_jsonl_indices(self):
        ...
        # 安全使用tqdm
        try:
            iterator = tqdm(jsonl_files, desc="加载索引")
        except:
    ...
    def _load_json_metadata(self):
        ...
        # 安全使用tqdm
        try:
            iterator = tqdm(json_files, desc="加载元数据")
        except:
            iterator = json_files
    ...
    def _load_xml_annotations(self):
        ...
        # 安全使用tqdm
        try:
            iterator = tqdm(xml_files, desc="加载XML")
        except:
            iterator = xml_files
    ...
    def _process_data(self):
        ...
        # 安全使用tqdm
        try:
            iterator = tqdm(self.raw_data['indices'].keys(), desc="处理对话")
        except:
            iterator = self.raw_data['indices'].keys()
            
        for dialogue_id in iterator:
