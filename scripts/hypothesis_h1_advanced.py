...
class H1ThreeLevelAnalysisV2:
    ...
    def _preprocess_data(self):
        ...
        if 'speaker_role' in self.data.columns:
            ...
            # 如果映射失败，使用默认值
            if self.data['speaker_id'].isna().any():
                logger.warning("部分speaker_role值无法映射，使用默认值")
                self.data['speaker_id'] = self.data['speaker_id'].fillna(0).astype(int)
        else:
            # 如果没有speaker_role，基于turn_id创建（备用方案）
            logger.warning("未找到speaker_role列，基于turn_id创建speaker标识")
            self.data['speaker_id'] = self.data.groupby('dialogue_id')['turn_id'].transform(
                lambda x: x % 2
            )
        
        # 创建角色变量
        self.data['role'] = self.data['speaker_id']  # 0=客户, 1=服务提供者
        
        # 创建唯一的说话人ID（使用speaker_role如果存在）
        if 'speaker_role' in self.data.columns:
    ...
    def _fit_null_model(self, data):
        """拟合三层嵌套空模型"""
        logger.info("拟合三层嵌套空模型...")
        
        # 首先尝试使用独立的三层ICC分析获得准确的方差分解
        try:
            # 导入三层ICC分析模块
            from three_level_icc_python import ThreeLevelICCAnalysis
            
            logger.info("使用独立的三层ICC分析...")
            icc_analyzer = ThreeLevelICCAnalysis()
            icc_analyzer.data = data.copy()
            icc_results = icc_analyzer.calculate_variance_components_anova()
            
            if icc_results and 'variance_components' in icc_results:
                var_components = icc_results['variance_components']
                var_residual = var_components.get('residual', 0)
                var_speaker = var_components.get('speaker', 0)
                var_dialogue = var_components.get('dialogue', 0)
                
                logger.info(f"✓ 三层ICC分析成功")
                logger.info(f"方差分解 - 残差: {var_residual:.4f}, 说话人: {var_speaker:.4f}, 对话: {var_dialogue:.4f}")
                
                # 保存ICC结果
                self._calculate_nested_icc(var_residual, var_speaker, var_dialogue)
                
                # 继续拟合标准混合模型用于后续分析
                try:
                    # 使用statsmodels拟合基础模型（用于后续分析）
                    ...
                    logger.info("✓ 基础模型拟合成功")
                except Exception as e:
                    logger.warning(f"基础模型拟合失败: {e}")
                
                return
        except ImportError:
            logger.warning("无法导入三层ICC分析模块，使用备选方案")
        except Exception as e:
            logger.warning(f"三层ICC分析失败: {e}, 使用备选方案")
        ...
        try:
            ...
            # 从两个模型估计方差成分（注意：这不是真正的三层模型）
            var_speaker = float(m0_speaker_fit.cov_re.iloc[0, 0])
            var_dialogue = float(m0_dialogue_fit.cov_re.iloc[0, 0])
            var_residual = float(m0_speaker_fit.scale)
            
            logger.info("✓ 两层模型成功（备选方案）")
            self._calculate_nested_icc(var_residual, var_speaker, var_dialogue)
                
        except Exception as e:
            logger.error(f"空模型失败: {e}")
    
    def _calculate_nested_icc(self, var_residual, var_speaker, var_dialogue):
        """计算三层嵌套模型的ICC"""
        try:
            # 计算总方差
            ...
            # 计算各层ICC（占总方差的比例）
            icc_speaker = var_speaker / var_total if var_total > 0 else 0
            icc_dialogue = var_dialogue / var_total if var_total > 0 else 0
            icc_cumulative = (var_speaker + var_dialogue) / var_total if var_total > 0 else 0
            
            # 计算方差百分比
            pct_residual = (var_residual / var_total) * 100 if var_total > 0 else 0
            pct_speaker = (var_speaker / var_total) * 100 if var_total > 0 else 0
            pct_dialogue = (var_dialogue / var_total) * 100 if var_total > 0 else 0
            
            logger.info(f"ICC - 说话人层: {icc_speaker:.3f}, 对话层: {icc_dialogue:.3f}, 累积: {icc_cumulative:.3f}")
            logger.info(f"方差百分比 - 残差: {pct_residual:.1f}%, 说话人: {pct_speaker:.1f}%, 对话: {pct_dialogue:.1f}%")
            ...
            self.results['icc'] = {
                ...
                'variance_components': {
                    ...
                    'speaker_variance': var_speaker,
                    'dialogue_variance': var_dialogue,
