...
class H4HypothesisAdvancedAnalysis:
    ...
    def _analyze_role_differences(self):
        ...
        if len(sp_distances) > 0 and len(c_distances) > 0:
            ...
            # 计算效应量（Cohen's d）
            cohens_d = (sp_distances.mean() - c_distances.mean()) / np.sqrt(
                (sp_distances.std()**2 + c_distances.std()**2) / 2
            )
        else:
            logger.warning("数据不足，无法进行t检验")
...
def main():
    """主函数"""
    # 创建高级分析器
    analyzer = H4HypothesisAdvancedAnalysis(language='zh')
